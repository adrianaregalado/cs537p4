...
int getpinfo(struct pstat *){
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      ...
      pstat->inuse[i] = p->inuse[i];
      pstat->pid[i] = p->pid[i];
      pstat->timeslice[i] = p->timeslice[i];
      pstat->compticks[i] = p->compticks[i];
      pstat->schedticks[i] = p->schedticks[i];
      pstat->sleepticks[i] = p->sleepticks[i];
      pstat->switches[i] = p->switches[i];
      i++;
  }
  return -1;
}
